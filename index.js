...
function indekseGoreFenomen(dizi, indeks) {
  const secilenFenomen = dizi[indeks];
  const result = `${indeks}. indekste bulunan fenomen: ${secilenFenomen.profile}`;
  return result;
}
...
function fenomenSil(array, indeks) {
  let newArray = [...array];
...
function fenomenEkle(dizi, sayi, profil, takipciSayisi, gönderimSayisi, platform) {
  let yeniDizi = [...dizi];
  const yeniProfil = {
    "number": sayi,
    "profile": profil,
    "followers": takipciSayisi,
    "posts": gönderimSayisi,
    "platform": platform
  }
  yeniDizi.push(yeniProfil);
  return yeniDizi;
}
...
function platformaGoreCokGonderiYapanFenomen(birDizi, platform) {
  let maxPost= 0;
  let maxPostProfile = "";
  for (let i=0; i <birDizi.length; i++){
    const item = birDizi[i];
    if(item.platform === platform && maxPost < item.posts && item.posts!== "NA" ){
      maxPost= item.posts;
